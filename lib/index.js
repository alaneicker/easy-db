/*
  TODOS: Add the following methods
  - updateMany - updates based on one or more filter matches
  - deleteMany - deletes based on one or more filter matches
*/
...
class EasyDB {
   ...
   * @param  {String} arg1 database table name
   * @param  {Array}  arg2 Columns to be returned
   * @param  {Object} arg2 Filter for WHERE clause. Must be unique and pertain to single record
   * @return {Object}      Result object containing query result. Contains error response if error was thrown
   */
  async selectOne(tableName, tableColumns, filters) {
    try {
      const db = await this.dbPromise;
      const columns = tableColumns.join(', ');
      
      if (filters === undefined) {
        throw new Error('Expected at least 1 filter but recieved 0');
      }

      const whereClauseParams = `WHERE ${
        Object.entries(filters).map(filter => {
          return `${filter[0]} = ${filter[1]}`;
        }).join(' AND ')
      }`;
   ...
   */
  async updateById(tableName, updates) {
    try  {
      const db = await this.dbPromise;
   ...
   */
  async deleteById(tableName, ids) {
    try {
      const db = await this.dbPromise;
