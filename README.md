...
```javascript
const result = await db.select('Users', ['name','email']);
```
...
const response = await db.select('Products', ['*'])

// Returns all records with only specified columns
const response = await db.select('Products', ['categoryId', 'name', 'price'])
```
...
Selects a single record based on a single unique filter
...
const response = await db.selectOne('Products', ['*'], { categoryId: 100 });
```
...
const response = await db.insert('Products', {
  categoryId: 100,
...
const response = await db.update('Producta', {
  id: 33545,
  price: 1049.99,
});
...
const response = await db.delete('Products', 68282);

// Returns => { "deleted": 1 }

//  Deletes multiple records from a table
const response = await db.delete('Products', [68282, 33559, 26657]);
...
const procucts = async () => {
  try {
    const { data } = await db.select('Products', ['categoryId','name','price']);
    return data;
  } catch (err) {
    return { err };
  }
...
const product = async ({ id }) => {
  try {
    const { data } = await db.select('Products', ['*'], { id });
    return data;
  } catch (err) {
    return { err };
  }
...
const insertProduct = async ({ body }) => {
  try {
    const status = await db.insert('Products', body);
    return status;
  } catch (err) {
    return { err };
  }
...
const updateProduct = async ({ body }) => {
  try {
    const status = await db.update('Products', body);
    return status;
  } catch (err) {
    return { err };
  }
...
const deleteProduct = async ({ ids }) => {
  try {
    const status = await db.delete('Products', ids);
    return status;
  } catch (err) {
    return { err };
  }
